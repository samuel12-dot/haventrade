...
function useInView(opts = {}) {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      ...
      { threshold: 0.05, rootMargin: '0px 0px -20px 0px', ...opts }
    );
    obs.observe(el);
...
function MeetNeighbours({ navigate }) {
  ...
  return (
    <section ref={ref} className="mt-[72px]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        {ids.map((id, i) => {
          ...
          return (
            <div
              key={id}
              className={`reveal sd-${i + 1} ${inView ? "in-view" : ""} bg-surface border border-border rounded-2xl shadow-lift overflow-hidden cursor-pointer flex flex-col`}
              onClick={() => navigate("storefront", { id })}
            >
              <div className={`grad ${s.grad} h-[110px] relative`}>
                <div className="absolute top-3 right-3">
                  <MarketTicket label="MEMBER SINCE" value={s.since} rotation={rot} />
                </div>
              </div>
              <div className="pt-[30px] pb-[18px] px-[18px] flex flex-col gap-1.5 flex-1 relative">
                <div className="absolute -top-[20px] left-4">
                  <SellerAvatar seller={s} size="md" />
                </div>
                <div className="font-serif text-lg leading-[1.15]">{s.name}</div>
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mt-6">
        {steps.map((s, i) => (
          <div
            key={s.n}
            className={`reveal sd-${i + 1} ${inView ? "in-view" : ""} flex flex-col gap-[18px] p-2`}
          >
            <div className="self-start">
              <MarketTicket label={`STEP ${s.n}`} value={s.v} rotation={s.rot} lg />
            </div>
            <div className="font-serif text-2xl tracking-[-0.02em] mt-3">{s.title}</div>
            <div className="text-sm text-ink-muted leading-[1.55] max-w-[320px]">{s.body}</div>
          </div>
        ))}
      </div>
    </section>
...
function TrendingRail({ navigate, savedSet, toggleSave }) {
  ...
  return (
    <section ref={ref} className="mt-[88px]">
      <div className={`reveal ${inView ? "in-view" : ""}`}>
        <SectionHead
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {trending.map((id, i) => {
          ...
          return (
            <div key={id} className={`reveal sd-${i + 1} ${inView ? "in-view" : ""} relative`}>
    ...
    <section ref={ref} className="mt-[88px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
        {quotes.map((q, i) => (
          <div
            key={i}
            className={`reveal sd-${i + 1} ${inView ? "in-view" : ""} bg-surface border border-border rounded-2xl shadow-lift relative flex flex-col gap-[18px] ${q.tall ? "px-6 py-8" : "p-6"}`}
          >
            ...
            <div className="border-t border-dashed border-border pt-3.5 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="font-serif italic text-xs text-ink-subtle">{q.where}</div>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {rings.map(({ r, dash, sw }) => (
          <circle key={r} cx={cx} cy={cy} r={r} fill="none" stroke="#BFDBFE" strokeWidth={sw} strokeDasharray={dash || undefined} />
        ))}
        {lineIdxs.map((i) => (
          <line key={i} x1={cx} y1={cy} x2={nodes[i].x} y2={nodes[i].y} stroke="#BFDBFE" strokeWidth="0.55" />
        ))}
        {nodes.map((n, i) => (
        ...
        <text x={cx + rings[3].r + 8} y={cy + 4} fontSize="8" fill="#BFDBFE" fontFamily="'JetBrains Mono', monospace" letterSpacing="1.5">2KM</text>
        <text x={cx + 10} y={cy - 14}  fontSize="7" fill="#BFDBFE" fontFamily="'JetBrains Mono', monospace" letterSpacing="2">MAITAMA</text>
      </svg>
    </div>
  );
}
...
function OpenAStall({ navigate }) {
  ...
  return (
    ...
    >
      <div className="grad grad-hero-blue relative overflow-hidden" style={{ color: "#F4EFE8", minHeight: 460 }}>
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at 92% 95%, rgba(6,10,32,0.72) 0%, transparent 50%)", zIndex: 0 }} />
        <div className="absolute inset-0 pointer-events-none" style={{ background: "linear-gradient(90deg, rgba(5,10,25,0.20) 0%, rgba(5,10,25,0.08) 38%, transparent 70%)", zIndex: 1 }} />
        <SellerGrain />
        <SellerRadiusViz />

        <div className="relative max-w-[1440px] mx-auto grid grid-cols-1 md:grid-cols-[3fr_2fr] items-center px-6 sm:px-16 py-[56px] sm:py-[72px] gap-8" style={{ zIndex: 2 }}>
          <div>
            <div
              className={`reveal ${inView ? "in-view" : ""} font-mono mb-[22px]`}
              style={{ color: "rgba(227,201,143,0.95)", fontSize: 11, letterSpacing: "0.22em", fontWeight: 500, textShadow: "0 1px 2px rgba(0,0,0,0.18)" }}
            >
              FOR SELLERS
            </div>
            <h2
              className={`reveal sd-1 ${inView ? "in-view" : ""} font-serif text-[38px] sm:text-[54px] leading-[0.93] tracking-[-0.03em] mb-5 max-w-[540px]`}
              style={{ color: "#FFF6ED" }}
            >
              Open a stall on your street.
            </h2>
            <p
              className={`reveal sd-2 ${inView ? "in-view" : ""} font-serif italic text-[19px] sm:text-[22px] leading-[1.4] mb-10 max-w-[440px]`}
              style={{ color: "rgba(255,246,237,0.82)" }}
            >
              List in 3 minutes. Sell to neighbours by sundown.
            </p>

            <div className={`reveal sd-3 ${inView ? "in-view" : ""} flex gap-3 flex-wrap mb-10`}>
              {stats.map(([n, l], i) => {
                ...
                return (
                  ...
                  >
                    <span className="font-serif text-[26px] leading-none tracking-[-0.03em]" style={{ color: "#FFF6ED" }}>{n}</span>
                    <span className="font-mono" style={{ color: "rgba(227,201,143,0.95)", fontSize: 11, letterSpacing: "0.22em", fontWeight: 500, textShadow: "0 1px 2px rgba(0,0,0,0.18)" }}>{l}</span>
                  </div>
                );
              })}
            </div>

            <div className={`reveal sd-4 ${inView ? "in-view" : ""} flex flex-col items-start gap-4`}>
              ...
              <div className="flex items-center gap-3">
                <div className="flex -space-x-2">
                  {avatars.map(({ g, l }, idx) => (
                    <span key={g} className={`grad grad-${g} w-[28px] h-[28px] rounded-full inline-flex items-center justify-center font-serif italic text-canvas text-[11px]`} style={{ border: "1.5px solid rgba(244,239,232,0.22)", zIndex: avatars.length - idx }}>{l}</span>
                  ))}
                </div>
                <span className="font-serif italic text-[13px]" style={{ color: "rgba(244,239,232,0.58)" }}>No listing fees, ever.</span>
              </div>
            </div>
          </div>
        ...
        <div className="absolute top-8 right-9 z-[3] hidden sm:block" style={{ transform: "rotate(2deg)" }}>
...
function PressStrip() {
  const [ref, inView] = useInView();
  const press = [
    { name: "Punch Newspapers", cls: "font-serif font-semibold text-[22px] tracking-[-0.02em]" },
    { name: "TechCabal",        cls: "font-mono font-bold text-sm tracking-widest-2 uppercase" },
    { name: "Abuja.info",       cls: "font-sans font-medium text-lg tracking-[-0.01em]" },
    { name: "Channels TV",      cls: "font-sans font-bold text-[17px] tracking-[0.04em] uppercase" },
    { name: "The Cable",        cls: "font-serif italic font-medium text-[22px] tracking-[-0.015em]" },
  ];
  return (
    <section ref={ref} className="py-12 text-center">
      <div className={`reveal ${inView ? "in-view" : ""} font-mono text-[10px] text-ink-subtle tracking-widest-3 mb-6`}>
        AS SEEN IN
      </div>
      <div className="flex flex-wrap justify-center items-center gap-6 sm:gap-8">
        {press.map((p, i) => (
          <span key={p.name} className={`reveal sd-${i + 1} ${inView ? "in-view" : ""} ${p.cls} text-ink-subtle opacity-65`}>
            {p.name}
          </span>
        ))}
      </div>
...
function FAQ() {
  const [ref, inView] = useInView();
  const items = [
    { q: "How fast is delivery, really?",                          a: "Most orders inside 4 hours, by bike. You pick a delivery window at checkout — same-day until 19:00, otherwise tomorrow morning." },
    { q: "What happens if a seller can't fulfil their part?",       a: "We refund just their portion automatically. The rest of your order proceeds. You'll see this in your order tracking page." },
    { q: "Why only 2 km?",                                         a: "Because that's walking and cycling distance. Bigger radius means more inventory but less of the neighbourhood feeling. You can dial it down to 500 m if you prefer." },
    { q: "Are sellers verified?",                                   a: "Every seller goes through ID verification with BVN. Look for the green checkmark on their storefront." },
    { q: "Can I sell digital goods?",                               a: "Yes — design files, e-books, prints. They deliver instantly via download link." },
    { q: "What about returns?",                                     a: "14 days on physical goods, no questions asked. Digital sales are final." },
  ];
  const [open, setOpen] = useState(0);
  return (
    <section ref={ref} className="mt-[88px] mb-8 py-12 border-t border-dashed border-border-strong">
      <div className="grid grid-cols-1 md:[grid-template-columns:2fr_3fr] gap-8 md:gap-16 items-start">
        <div className={`reveal ${inView ? "in-view" : ""}`}>
          <div className="font-mono text-[10px] text-hearth tracking-widest-2 mb-3">FAQ</div>
        ...
        <div className={`reveal sd-1 ${inView ? "in-view" : ""} flex flex-col`}>
          {items.map((it, i) => {
            const isOpen = open === i;
            return (
              <div key={i} className={`${i === 0 ? "border-t border-dashed border-border-strong" : ""} border-b border-dashed border-border-strong`}>
                <button
                ...
                >
                  <span className="font-serif text-base sm:text-lg tracking-[-0.015em] leading-[1.3]">{it.q}</span>
                  <span
...
export default function LandingScreen({ navigate, savedSet, toggleSave }) {
  const [peekRef, peekInView] = useInView();

  return (
    <div className="page pt-6">
      {/* Bento hero */}
      <div className="grid grid-cols-1 md:grid-cols-[1.5fr_1fr] gap-4" style={{ gridTemplateRows: "auto auto" }}>
        ...
        >
          <div className="absolute top-7 right-8 z-[2] hidden sm:block anim-fade-in ad-6">
            <MarketTicket label="EST. 2026" value="Abuja" rotation={3} variant="cream" lg />
          </div>

          <div className="relative z-[1]">
            <div
              className="anim-fade-in ad-0 inline-flex items-center gap-3 mb-[22px] px-3.5 py-2 rounded-full"
            ...
            <div className="anim-fade-in ad-1">
              <StatusPill>247 NEIGHBOURS · LIVE NOW</StatusPill>
            </div>

            <h1 className="anim-fade-up ad-2 h-hero mt-5 mb-3 max-w-[580px]" style={{ color: "#FFFFFF" }}>
              Shop your street.
            </h1>

            <div
              className="anim-fade-up ad-3 font-serif italic text-[24px] sm:text-[32px] leading-[1.05] tracking-[-0.01em] mb-7"
            ...
            <p
              className="anim-fade-up ad-4 text-base max-w-[460px] mb-7 leading-[1.5]"
          ...
          <div className="anim-fade-up ad-5 relative z-[1]">
        ...
        {/* Right top — Pulse */}
        <div className="anim-slide-right ad-2">
          <PulsePanel />
        </div>

        {/* Right bottom — Radius */}
        <div
          className="anim-slide-right ad-3 grad grad-deepmoss rounded-[20px] p-[22px] relative overflow-hidden"
        ...
        >
          <div className="radius-rings">
            ...
            <span className="absolute w-3.5 h-3.5 rounded-full bg-saffron" style={{ boxShadow: "0 0 0 4px rgba(242,181,68,0.3)" }} />
          </div>
          <div className="relative z-[1] flex flex-col h-full justify-between">
            <span className="font-mono text-[10px]" style={{ color: "rgba(251,246,236,0.7)" }}>YOUR RADIUS</span>
            <div>
              <div className="font-serif text-[56px] leading-[0.95] text-canvas">
                2.0 <span className="text-[26px]">km</span>
              </div>
              <div className="font-serif italic text-base mt-1.5" style={{ color: "rgba(251,246,236,0.9)" }}>
      ...
      <div className="divider-ticket mt-14">
        <span className="font-mono text-[11px] text-hearth">A PEEK AT WHAT'S NEARBY</span>
      </div>
      <div ref={peekRef} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
        {[LISTINGS[0], LISTINGS[1], LISTINGS[10]].map((l, i) => (
          <div key={l.id} className={`reveal sd-${i + 1} ${peekInView ? "in-view" : ""}`}>
            <ListingCard
              listing={l}
